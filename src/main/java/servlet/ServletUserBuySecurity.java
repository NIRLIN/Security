...
public class ServletUserBuySecurity extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (num==1){
            response.sendRedirect(request.getContextPath()+"/servlet/ServletFindOneSecurity?securitys_id="+securitys_id);
        }else {
            response.sendRedirect(request.getContextPath()+"/servlet/ServletAllFind");
        }
    }
}
